...
void CalculateLaplacian(int Nx, int Ny, double *c, double *nabla,
			double deltax, double deltay)
{
	...
	for (j = 0; j < Ny; ++j) {
	    ...
	    if (j == (Ny - 1))
		N = i * Ny;
	    nabla[J] = temp1 * (c[E] + c[W] - 2 * c[J]) + temp2 * (c[N] + c[S] - 2 * c[J]);
	}
    }
}

void CalculateMu(int Nx, int Ny, double *g, double *nabla, double *mu,
		 double kappa)
{
    int i, j;
    int J;
